    ...
    for (auto state1 : get_states(order, 0))
    {
        for (auto state2 : get_states(order, 1))
        {
            for (auto state3 : get_states(order, 2))
            {
                ...
                if (props.results == ps::CORRECT && props.semantics == ps::INCORRECT && semantics == pl::THREE_LEVELS)
                    continue;
                ...
                if (props.results == ps::CORRECT && is_SM_X(props.semantics))
                    continue;
                if (props.results == ps::CORRECT && props.semantics == ps::INCORRECT && semantics == pl::SEMATICS_LEVELS_6)
                    continue;
...
std::string Grader::property_state_to_string(property_state state)
{
    switch (state)
    {
    case property_state::INVALID:
        return "invalid";
    case property_state::INCORRECT:
        return "Incorrect";
    case property_state::SM_1:
        return "SM_1";
    case property_state::SM_2:
    ...
    case property_state::SM_5:
        return "SM_5";
    case property_state::MINOR_INCORRECT:
        return "Minor incorrect";
    case property_state::CORRECT:
        return "correct";
    default:
        return "unknown";
    }
}
std::map<string, double> Grader::get_normalized_property_values(property_level sn_level, property_level sm_level, property_level rs_level, property_state results, property_state semantics, property_state syntax)
{
    // initialize the default max value, -1 which means the property is absent
    int max_syntax_outcome = -1;
    int max_semantics_outcome = -1;
    int max_results_outcome = -1;
    // outcomes in integer format normalize from 0 to 1
    double syntax_outcome = 0;
    double semantics_outcome = 0;
    double results_outcome = 0;

    // set the max values based on the levels
    // first syntax
    if (sn_level == property_level::TWO_LEVELS)
    {
        // binary levels, so either 0 or 1
        max_syntax_outcome = 2;
        syntax == property_state::CORRECT ? syntax_outcome = 1 : syntax_outcome = 0;
    }
    else if (sn_level == property_level::THREE_LEVELS)
    {
        max_syntax_outcome = 3;
        switch (syntax)
        {
        case property_state::CORRECT:
            syntax_outcome = 1;
            break;
        case property_state::MINOR_INCORRECT:
            syntax_outcome = 2.0 / 3.0;
            break;
        case property_state::INCORRECT:
            syntax_outcome = 1.0 / 3.0;
            break;
        }
    }
    // then semantics
    if (sm_level == property_level::TWO_LEVELS)
    {
        max_semantics_outcome = 2;
        semantics == property_state::CORRECT ? semantics_outcome = 1 : semantics_outcome = 0;
    }
    else if (sm_level == property_level::THREE_LEVELS)
    {
        max_semantics_outcome = 3;
        switch (semantics)
        {
        case property_state::CORRECT:
            semantics_outcome = 1;
            break;
        case property_state::MINOR_INCORRECT:
            semantics_outcome = 2.0 / 3.0;
            break;
        case property_state::INCORRECT:
            semantics_outcome = 1.0 / 3.0;
            break;
        }
    }
    else if (sm_level == property_level::SEMATICS_LEVELS_6)
    {
        max_semantics_outcome = 8; // SM_1 to SM_5 + minor incorrect + correct
        switch (semantics)
        {
        case property_state::CORRECT:
            semantics_outcome = 1;
            break;
        case property_state::MINOR_INCORRECT:
            semantics_outcome = 7.0 / 8.0;
            break;
        case property_state::SM_5:
            semantics_outcome = 6.0 / 8.0;
            break;
        case property_state::SM_4:
            semantics_outcome = 5.0 / 8.0;
            break;
        case property_state::SM_3:
            semantics_outcome = 4.0 / 8.0;
            break;
        case property_state::SM_2:
            semantics_outcome = 3.0 / 8.0;
            break;
        case property_state::SM_1:
            semantics_outcome = 2.0 / 8.0;
            break;
        case property_state::INCORRECT:
            semantics_outcome = 1.0 / 8.0;
            break;
        }
    }
    // then results
    if (rs_level == property_level::TWO_LEVELS)
    {
        max_results_outcome = 2;
        results == property_state::CORRECT ? results_outcome = 1 : results_outcome = 0;
    }
    else if (rs_level == property_level::THREE_LEVELS)
    {
        max_results_outcome = 3;
        switch (results)
        {
        case property_state::CORRECT:
            results_outcome = 1;
            break;
        case property_state::MINOR_INCORRECT:
            results_outcome = 2.0 / 3.0;
            break;
        case property_state::INCORRECT:
            results_outcome = 1.0 / 3.0;
            break;
        }
    }
    std::map<string, double> property_values;
    property_values["syntax"] = syntax_outcome;
    property_values["semantics"] = semantics_outcome;
    property_values["results"] = results_outcome;
