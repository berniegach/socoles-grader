...
export async function GET(req: NextRequest) {
    try {
        ...
        let contextCourseId: string | null = null;
        if (payload.role === 'instructor') {
            contextInstructorId = payload.sub;
            contextCourseId = requestedCourseId ? String(requestedCourseId) : null;
            ...
            contextCourseId = (payload.courseId ? String(payload.courseId) : null);
            const run = <T>(fn: () => Promise<T>) => contextCourseId
                ? withCourseContext(instructorId, contextCourseId, fn)
        ...
        const rows = await (contextCourseId
            ? withCourseContext(contextInstructorId!, contextCourseId, async () => {
                if (rosterId) {
            ...
            : withInstructorContext(contextInstructorId!, async () => {
                if (rosterId) {
                    const active = await query<any>(
                        `SELECT i.id, i.roster_id as "rosterId", i.token, i.email, i.name, i.expires_at as "expiresAt", i.used_at as "usedAt"
                         FROM invites i
...
export async function POST(req: NextRequest) {
    try {
        ...
        const { rosterId, email, name, expiresInDays, courseId } = body || {};
        ...
        if (payload.role === 'instructor') {
            contextInstructorId = payload.sub;
            contextCourseId = courseId ? String(courseId) : null;
        } else if (payload.role === 'student' && payload.instructorId) {
            const instructorId = String(payload.instructorId);
            const meId = String(payload.sub);
            contextCourseId = (payload.courseId ? String(payload.courseId) : null);
        ...
        // Fetch a human-readable course name for the invite email
        let className = '';
        try {
            if (contextCourseId) {
                const c = await query<{ name: string }>(`SELECT name FROM courses WHERE id=$1 LIMIT 1`, [contextCourseId]);
                className = c.rows[0]?.name?.trim() || 'your course';
            } else {
                ...
                className = settingsRes.rows[0]?.course_name?.trim() || 'your course';
            }
        } catch {
