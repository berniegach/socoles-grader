...
export async function GET(req: NextRequest) {
    try {
        ...
        if (payload.role === 'student') {
            ...
            const me = await run(() => query<{ evaluator: boolean }>(`SELECT evaluator FROM roster WHERE id=$1 AND owner_id = current_setting('app.current_instructor')::uuid LIMIT 1`, [meId]));
...
export async function POST(req: Request) {
    try {
        ...
        const assignmentTitle = body.assignment;
        ...
        const { rows: aRows } = await run(() => query<{ id: string }>(
            `SELECT id FROM assignments WHERE title=$1 AND owner_id = current_setting('app.current_instructor')::uuid AND course_id = current_setting('app.current_course', true)::uuid ORDER BY created_at DESC LIMIT 1`, [assignmentTitle]
        ));
        ...
        const { rows } = await run(() => query<Submission>(
...
export async function PATCH(req: Request) {
    try {
        ...
        const run = <T>(fn: () => Promise<T>) => courseId
            ? withCourseContext(instructorId, courseId, fn)
            : withInstructorContext(instructorId, fn);
        const { rows } = await run(() => query<Submission>(`UPDATE submissions SET grade=COALESCE($2, grade), status=COALESCE($3,status) WHERE id=$1 AND owner_id = current_setting('app.current_instructor')::uuid AND course_id = current_setting('app.current_course', true)::uuid RETURNING id, student, assignment_id as "assignmentId", date, grade::float8 as grade, status`, [id, grade, status]));
...
export async function DELETE(req: NextRequest) {
    try {
        ...
        const run = <T>(fn: () => Promise<T>) => courseId
            ? withCourseContext(instructorId, courseId, fn)
            : withInstructorContext(instructorId, fn);
        const { rows: existing } = await run(() => query<{ id: string; status: string }>(`SELECT id, status FROM submissions WHERE id=$1 AND owner_id = current_setting('app.current_instructor')::uuid AND course_id = current_setting('app.current_course', true)::uuid`, [id]));
        ...
        await run(() => query(`DELETE FROM submissions WHERE id=$1 AND owner_id = current_setting('app.current_instructor')::uuid AND course_id = current_setting('app.current_course', true)::uuid`, [id]));
