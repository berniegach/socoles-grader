    ...
    async function ensureSubmissionImmediate() {
        ...
        try {
            ...
            if (sRes.ok) {
                ...
                const mine = (all || []).filter((s: any) => s.assignmentId === assignment.id);
    ...
    useEffect(() => {
        ...
        async function checkLock() {
            ...
            try {
                ...
                const existing = (all || []).find((s: any) => s.assignmentId === assignment.id && (s.status === 'Submitted' || s.status === 'Auto-graded' || s.status === 'Needs review'));
                if (existing) {
