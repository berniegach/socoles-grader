import { withInstructorContext, query } from '@/lib/db';
...
export async function computeAssignmentImprovement({ instructorId, assignmentId, studentName = '', studentEmail = '' }: ImprovementInput): Promise<ImprovementStats> {
    // Resolve latest submission for this assignment+student (name or email)
    const { rows: subRows } = await withInstructorContext(instructorId, () => query<{ id: string; grade: number | null }>(
    ...
    if (submissionId) {
        const { rows: firstRows } = await withInstructorContext(instructorId, () => query<{ question_id: string; grade: number }>(
        ...
        const { rows: finalRows } = await withInstructorContext(instructorId, () => query<{ question_id: string; grade: number }>(
            `SELECT DISTINCT ON (question_id) question_id, grade::float8 as grade
         FROM question_submission_attempts
        WHERE owner_id = current_setting('app.current_instructor')::uuid
          AND submission_id = $1
        ...
        const { rows: cntRows } = await withInstructorContext(instructorId, () => query<{ cnt: number }>(
...
export async function computeSurveyEligibility({ instructorId, assignmentId, studentName = '', studentEmail = '' }: ImprovementInput): Promise<EligibilityResult> {
    const { rows: subRows } = await withInstructorContext(instructorId, () => query<{ id: string }>(
        `SELECT id
       FROM submissions s
      WHERE s.owner_id = current_setting('app.current_instructor')::uuid
        AND s.assignment_id = $1
    ...
    const { rows } = await withInstructorContext(instructorId, () => query<{ question_id: string; min_attempt: number; max_attempt: number; first_grade: number | null; max_grade: number | null }>(
        `SELECT question_id,
